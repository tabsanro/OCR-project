...
def run_ocr(image):
    ...
    for (bbox, text, prob) in results:
        ...
        if text:
            globals()['b_{}'.format(i)] = Button(tl[0], tl[1], tr[0] - tl[0], bl[1] - tl[1])
            engine.setProperty('rate', 200)
            engine.save_to_file(text, 'text.mp3')
            engine.runAndWait()
            sound = pygame.mixer.Sound("text.mp3")
            globals()['b_{}'.format(i)].sound_1 = sound
            engine.setProperty('rate', 300)
            engine.save_to_file(text, 'text.mp3')
            engine.runAndWait()
            sound = pygame.mixer.Sound("text.mp3")
            globals()['b_{}'.format(i)].sound_2 = sound
            engine.setProperty('rate', 400)
            engine.save_to_file(text, 'text.mp3')
            engine.runAndWait()
            sound = pygame.mixer.Sound("text.mp3")
            globals()['b_{}'.format(i)].sound_3 = sound
            engine.setProperty('rate', 200)
            ...
            if idx == 1:
                ...
                if on_11 == 0:
                    ...
                    if end - start > 0.08:
                        ...
                        if diff >= 7:
                            scale_ratio += 0.1
                            if int(scale_ratio <= 250):
                                pygame.mixer.stop()
                                sound = pygame.mixer.Sound(f"./scale_tts/{int(scale_ratio*100)}.mp3")
                                sound.play()
                        elif diff <= -7:
                            scale_ratio -= 0.1
                            if int(scale_ratio >= 50):
                                pygame.mixer.stop()
                                sound = pygame.mixer.Sound(f"./scale_tts/{int(scale_ratio * 100)}.mp3")
                                sound.play()
                        on_11 = 0
                ...
                if rate != 1:
                    ...
                    pygame.mixer.stop()
                    sound = pygame.mixer.Sound("normal_speed.mp3")
                    sound.play()
            elif idx == 3:
                if rate != 2:
                    rate = 2
                    ...
                    pygame.mixer.stop()
                    sound = pygame.mixer.Sound("fast_speed.mp3")
                    sound.play()
            elif idx == 4:
                if rate != 3:
                    rate = 3
                    ...
                    pygame.mixer.stop()
                    sound = pygame.mixer.Sound("very_fast_speed.mp3")
                    sound.play()
            else:
                on_11 = 0
